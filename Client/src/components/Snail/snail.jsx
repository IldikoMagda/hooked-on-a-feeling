...
function SnailAnimation() {
  useEffect(() => {
    ...
    const snail = document.getElementById('snell');
    const leftEye = document.getElementById('eye1');
    const rightEye = document.getElementById('eye2'); // Replace 'Vector' with the actual ID of the path.

    if (snail) {
        anime({
          targets: snail,
        keyframes: [
            // { rotateY: 180, duration: 1000, easing: 'linear' }, // Flip the snail
            { translateX: -100, duration: 4000, easing: 'linear' }, // Move to the left
            { rotateY: 180, duration: 1000, easing: 'linear' }, // Flip the snail
            { translateX: 120, duration: 3000, easing: 'linear' }, // Move to the right
            { rotateY: 0, duration: 1000, easing: 'linear' }, // Flip the snail back
            { translateX: -100, duration: 3000, easing: 'linear' }, // Move to the left
            { rotateY: 180, duration: 1000, easing: 'linear' }, // Flip the snail
            { translateX: -100, duration: 4000, easing: 'linear' }, // Move to the left
            { rotateY: 180, duration: 1000, easing: 'linear' }, // Flip the snail
            // { translateX: -100, duration: 2000, easing: 'linear' }, // Move to the left
          ],
          loop: true,
        });
    
        // Animate the eyes' position (modify the values and easing)
        anime({
          targets: [leftEye, rightEye],
          translateY: [
            { value: 0, duration: 1000, easing: 'linear' },
            { value: 10, duration: 1000, easing: 'linear' },
            { value: 0, duration: 1000, easing: 'linear' },
          ],
          loop: true,
        });
